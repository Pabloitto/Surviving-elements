...
Panel = function(){
	...
	function paintBg(){
		...
		for (w=0; w<canvas.width;w+=background.width) {
	        for (h=0; h < canvas.height; h += background.height) {
	...
	function drawRotatedImage(shape) { 
		var image = shape.getImage();
		ctx.save();
		ctx.clearRect(0, 0, canvas.width, canvas.height);
		ctx.translate(shape.x, shape.y);
		ctx.rotate(shape.angle * Math.PI/180);
		ctx.drawImage(image, !shape.frame ? 0 : (shape.width * shape.frame), 
				0, 
				shape.width,
           		shape.height,
				-(shape.width/2), 
	...
	function isNotInCanvas(s){
		return ((s.x >= canvas.width || s.x < 0) || (s.y >= canvas.height || s.y < 0));
	}
	function bindEvents(){
